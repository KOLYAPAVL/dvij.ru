...
### 3. Приложение ораганизатора

3.1 Отдельное приложения для организатора. Те же самые страницы находятся в админке на сайте. Админка организатора будет находится на поддомене

3.2 Мои события. Тут будет список событий организатора. Так же возможность создать новое
    - 3.2.1 Создать событие. Тут создается событие. Тут можно добавлять билеты, товары
    - 3.2.2 После создания оно уходит на модерацию.
    
3.3 Событие. Тут мы можешь настраиваить или менять событие и пободное. Тут можно добавлять / менять билеты, товары

3.4 Мой кошелек. Страница с полученными деньги от выступления. Тут будет история выводов и вывод средств на Расчетный счет / карточку / ИП

3.5 Аналитика. Страница с отзывами / продажами и тд.

3.6 Чат. Здесь будет чат куда люди пишут организатору, а он отвечает на их сообщения.

3.7 Добавить работника. Здесь можно добавить нового пользователя в свою команду и дать ему разного рода возможности.

3.8 Проверка билетов. Здесь открывается сканер QR и проверяет билет участника. 

3.9 Мой аккаунт. Настройки аккаунта и тд

3.10 Поддержка. Здесь будет поддержка для организаторов

(p.s. Для организатора надо детально все проработать)

### 4. Веб сайт

3.1 Главная страница. Нибольшой лендинг со списком событий и фильтром (как в приложении)

3.2 Страница события. Такая же как в приложение

3.3 Поддержка. Поддержка для пользователей купивших билеты

(p.s. Авторизация на сайте не предусматривается для участников)

### 5. Осуществление продаж 

3.1 Продажа доступна по таким способам оплаты
- Карта
- Qiwi
- Yoomoney
- Мобильные платежи

3.2 После покупки билет высылыется на почту и добавляется в приложение. 

3.3 Билет представляет собой QR код со всеми данными. Покупка допов аналогичная.

3.4 При любых изменениях по событию отправляется уведомление на почту и в приложение
...
1. Backend
- За основной прием запросов отвечает Django Restful API.
- Celery - таймер
- Node.JS + Socket.io - уведомления
- Обработчик запросов: nginx
- Docker machine для развертывания
